...
. For IETF dual-publishing, rename `sources/draft-ietf-example-00.adoc` to match your IETF draft name.
...
├── metanorma.release.yml         # Routes cc-* and draft-* to channels
└── sources/
    ├── cc-XXXXX.adoc             # CalConnect document
    ├── draft-ietf-example-00.adoc # IETF internet-draft
    ├── sections/                 # Shared + CC-specific sections
    │   ├── 00-abstract.adoc
    │   ├── 01-scope.adoc
    │   ├── 02-normative-references.adoc
    │   └── 03-terms.adoc
    └── sections-ietf/            # IETF-specific sections
        ├── 02-conventions.adoc
        └── 99-references.adoc
```

Both documents can share sections from `sections/` while having format-specific sections in `sections-ietf/`.
...
|`internet-draft` |IETF internet-draft |Standards, Specifications & Reports
|===
...
* `:mn-document-class: ietf` — uses IETF slug and formatting rules
* `:name:` — the IETF draft name (used for slug derivation)
* `:ipr:`, `:area:`, `:intended-series:`, `:workgroup:` — IETF-specific metadata
* `:mn-output-extensions:` includes `rfc` and `txt` instead of `pdf`
...
For dual-publishing (CalConnect + IETF to the same channel):
